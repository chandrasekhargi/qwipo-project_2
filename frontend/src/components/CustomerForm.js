...
export default function CustomerForm(){
  ...
  useEffect(()=>{
    if(id){
      axios.get(`http://localhost:4000/api/customers/${id}`).then(r=>{
  ...
  const submit = async (e) => {
    ...
    try{
      if(id){
        await axios.put(`http://localhost:4000/api/customers/${id}`, { firstName, lastName, phone, email, accountType });
        alert('Updated');
      } else {
        await axios.post('http://localhost:4000/api/customers', { firstName, lastName, phone, email, accountType });
        alert('Created');
      }
      navigate('/');
    }catch(err){
