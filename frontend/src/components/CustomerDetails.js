...
export default function CustomerDetails(){
  ...
  const fetch = async () => {
    const res = await axios.get(`https://qwipo-server-2.onrender.com/api/customers/${id}`);
  ...
  const addAddress = async (e) => {
    ...
    await axios.post(`https://qwipo-server-2.onrender.com/api/customers/${id}/addresses`, { line1, city, state: stateV, pincode });
  ...
  const removeAddr = async (aid) => {
    ...
    await axios.delete(`https://qwipo-server-2.onrender.com/api/addresses/${aid}`);
    fetch();
  };

  const fetchOrders = async () => {
    const res = await axios.get(`https://qwipo-server-2.onrender.com/api/customers/${id}/orders`);
    setOrders(res.data.orders || []);
  };

  const fetchPayments = async () => {
    const res = await axios.get(`https://qwipo-server-2.onrender.com/api/customers/${id}/payments`);
